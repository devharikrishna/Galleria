...
import com.irah.galleria.ui.gallery.components.DragSelectReceiver
import com.irah.galleria.ui.gallery.components.FastScroller
...
fun AlbumDetailGridContent(
    ...
    if (settings.albumDetailViewType == GalleryViewType.GRID) {
        ...
        LaunchedEffect(media, itemSizePx) {
            ...
            .map { it / settings.albumDetailGridCount } 
                    ...
                    for (item in itemsToPreload) {
                        if (preloadedIds.add(item.id)) {
                             val request = ImageRequest.Builder(context)
                                .data(item.uri)
                                .size(itemSizePx)
                                .memoryCacheKey("${item.id}_$itemSizePx")
                                .diskCacheKey("${item.id}_$itemSizePx")
                                .build()
        ...
        DragSelectReceiver(
            ...
            FastScroller(
                ...
                LazyVerticalStaggeredGrid(
                    ...
                    columns = StaggeredGridCells.Fixed(settings.albumDetailGridCount),
                    modifier = Modifier.fillMaxSize().then(dragModifier),
                    contentPadding = PaddingValues(2.dp),
                    horizontalArrangement = Arrangement.spacedBy(2.dp),
